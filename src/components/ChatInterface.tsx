...
export const ChatInterface: React.FC<ChatInterfaceProps> = ({
  ...
  const handleImageSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > 12 * 1024 * 1024) {
      alert(isPt ? 'A imagem excede 12MB. Por favor escolha uma foto mais leve.' : 'La fotografía excede los 12MB. Por favor seleccione una imagen más liviana.');
      return;
    }
  ...
  const handleSendMessage = async (textToSend?: string) => {
    ...
    const userMsg: ChatMessage = {
      id: userMessageId,
      sender: 'user',
      text: text || (isPt ? 'Fotografia anexada para peritagem técnica.' : 'Fotografía adjunta para evaluación pericial.'),
      timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
    ...
    try {
      ...
      if (isSpeechEnabled && 'speechSynthesis' in window) {
        ...
        utterance.lang = isPt ? 'pt-BR' : 'es-ES';
        utterance.rate = 1.0;
  ...
  const handleResetChat = () => {
    if (window.confirm(isPt ? 'Deseja reiniciar a conversa com o conserje?' : '¿Desea reiniciar la conversación con el conserje técnico?')) {
  ...
  return (
    <div className="max-w-4xl mx-auto py-4 px-2 sm:px-4">
      ...
      <div className="bg-[#0b0e13] border border-[#2b2416] rounded-2xl shadow-2xl overflow-hidden flex flex-col h-[78vh] min-h-[580px] max-h-[820px]">
        ...
        <div className="bg-gradient-to-r from-[#12161f] via-[#0e1117] to-[#12161f] p-4 sm:p-5 border-b border-[#251f14] flex items-center justify-between">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h2 className="font-serif-luxury font-bold text-white text-sm sm:text-base tracking-wider">
                  {isPt ? 'Concierge Técnico do Atelier' : 'Concierge Técnico del Atelier'}
                </h2>
              ...
              <p className="text-xs text-slate-400 font-light flex items-center gap-1.5 mt-0.5">
                ...
                <span className="text-amber-400/90">{isPt ? 'Atendimento Exclusivo' : 'Atención de Alta Gama'}</span>
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              title={isSpeechEnabled ? (isPt ? 'Desativar voz' : 'Desactivar voz') : (isPt ? 'Ativar voz' : 'Activar lectura por voz')}
            >
            ...
            <button
              ...
              title={isPt ? 'Reiniciar conversa' : 'Reiniciar conversación'}
            >
                ...
                <div className={`max-w-[85%] sm:max-w-[75%] space-y-2`}>
                  ...
                  >
                    ...
                    {!isUser && (
                      <>
                        <span>·</span>
                        <span className="text-amber-400/80 font-medium">{isPt ? 'Atelier Oficial' : 'Atelier Oficial'}</span>
                      </>
                    )}
                  </div>
                </div>
            ...
            <div className="flex gap-3 justify-start items-center">
              ...
              <div className="bg-[#12161f] border border-[#2b2316] rounded-2xl rounded-tl-sm px-4 py-3 flex items-center gap-2 shadow-lg">
                ...
                <span className="text-xs text-slate-400 ml-1 font-serif-luxury tracking-wide">
                  {isPt ? 'Consultando especificações do atelier...' : 'Consultando especificaciones del atelier...'}
                </span>
              </div>
            </div>
          )}
        ...
        {/* Quick Action Suggestion Chips (Luxury edition) */}
        <div className="bg-[#080a0e] px-4 py-2.5 border-t border-[#1e1910] overflow-x-auto scrollbar-none flex items-center gap-2">
          <span className="text-[10px] uppercase font-bold tracking-widest text-amber-400/70 font-serif-luxury flex-shrink-0 mr-1 flex items-center gap-1">
            ...
            {isPt ? 'Ações Rápidas:' : 'Acciones VIP:'}
          </span>
        ...
        <div className="bg-[#0b0e13] p-3 sm:p-4 border-t border-[#231d13]">
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <span className="text-xs text-white font-medium block">
                    {isPt ? 'Foto anexada para peritagem' : 'Fotografía adjunta para evaluación'}
                  </span>
                  <span className="text-[10px] text-emerald-400">
                    {isPt ? 'Pronta para envio pericial' : 'Lista para análisis pericial'}
                  </span>
                </div>
              </div>
              <button
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="p-3 rounded-xl bg-[#141822] hover:bg-[#1c2230] border border-slate-800 hover:border-amber-500/40 text-slate-400 hover:text-amber-300 transition"
              title={isPt ? 'Anexar foto do dano' : 'Adjuntar fotografía del daño para peritaje'}
            >
            ...
            >
              ...
              <span className="hidden sm:inline text-xs font-serif-luxury tracking-wider">{isPt ? 'Enviar' : 'Consultar'}</span>
            </button>
          </div>
        </div>
      </div>
